	...
	<script type="text/javascript">
		$(function(){
			...
			getusers = function(){
				...
				$.getJSON(_base_url + '/logistik/accessusers', {}, function(json){
			...
			edit = function(tipe, id){
				$.post(_base_url + '/logistik/editaksesuser', {
					tipe : tipe,
					id : id
				}, function(json){
					onDataCancel();
			...
			del = function(id){
				...
				$.post(_base_url + '/logistik/delaccesgudang', {
					id : id
				}, function(json){
					$('.user-' + json.id).remove();
					getusers();
			...
			allusergudang = function(page){
				...
				$.ajax({
					type 	: 'GET',
					url 	: _base_url + '/logistik/getusergudang',
					data 	: {
						page 	: page,
						nama 	: $nama,
						tipe 	: $tipe,
									...
									<tr class="user-{{ $user->id_akses_gudang }}">
										...
										<td>
											<label for="obat{{ $user->id_akses_gudang }}">
												<input type="radio" value="1" name="tipe{{ $user->id_akses_gudang }}" onclick="edit(1, {{ $user->id_akses_gudang }});" id="obat{{ $user->id_akses_gudang }}" {{ $user->tipe == 1 ? 'checked="checked"' : '' }}> 
												<small>Obat</small>
											</label>
											
											<label for="barang{{ $user->id_akses_gudang }}">
												<input type="radio" value="2" name="tipe{{ $user->id_akses_gudang }}" onclick="edit(2, {{ $user->id_akses_gudang }});" id="barang{{ $user->id_akses_gudang }}" {{ $user->tipe == 2 ? 'checked="checked"' : '' }}>
												<small>barang</small>
											</label>
										</td>
										<td>
				...
				<div class="grid-body no-border">
					...
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<div class="form-group">
	                  	...
	                  	<div class="form-group">
	                  		<select style="width:100%" required name="tipe">
		                  		<option value="1">Gudang Obat</option>
		                  		<option value="2">Gudang Barang</option>
		                  	</select>
	                  	</div>
	                  	...
	                  	<div class="form-group">
	                  		...
	                  		<a href="{{ url('/logistik') }}" class="btn btn-block btn-primary">Kembali</a>
	                  	</div>
