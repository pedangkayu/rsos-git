          ...
          <div class="col-sm-10">
            ...
            <div class="form-group">
              <label for="title">Kode Account *</label>
				  <div>
					<input type="text" value="{{ $data->no_coa }}" class="form-control" id="no_coa" name="no_coa" required="required" />
				  </div>
            </div>
            ...
            <div class="form-group">
			  ...
			  <!-- bagian ini tampilkan yang ada di tabel ref_coa saja, bertingkat sesuai dgn grup parent masing2 -->
                <select class="form-control" id="parent" name="idparent">
                  ...
                  <option value="{{ $menus->id_coa }}" {{ empty($data) ? '' : $menus->id_coa == $data->parent_id ? 'selected="selected"' : '' }}>- {{ $menus->nm_coa }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="form-group">
              <label></label>
				  ...
				  @if(Auth::user()->permission == 3)
					  <div>
						<button type="submit" class="btn btn-flat btn-primary">Update Account Grup</button>
