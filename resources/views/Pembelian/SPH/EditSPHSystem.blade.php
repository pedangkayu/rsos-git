...
@section('content')
	<form method="post" action="{{ url('/sph/editsphsystem') }}" id="simpan">
		<input type="hidden" name="_token" value="{{ csrf_token() }}">
		<input type="hidden" name="id" value="{{ $id }}">
		<input type="hidden" name="id_sph_grup" value="{{ $sph->id_sph_grup }}">
						...
						<table class="table table-striped">
							<thead>
								<tr>
									...
									<th width="35%" class="text-center">Opsi</th>
								</tr>
							</thead>
									...
									<tr title="{{ $item->nm_barang }}">
										...
										<td class="text-right">
											...
											<div class="input-group">
											  	<input type="number" class="form-control text-right" name="qty[]" value="{{ $item->qty }}">
											  	<span class="input-group-addon" id="sizing-addon2"><small>{{ $item->nm_satuan }}</small></span>
											</div>
										</td>
										<td>
